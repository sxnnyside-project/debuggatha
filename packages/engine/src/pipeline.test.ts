...
import { executeReview, toRepoRelative } from "./pipeline.js";
...
describe("executeReview", () => {
  it("runs a real files review end-to-end and persists the ledger", () => {
    ...
    expect(result.findings.some((f) => f.locations[0]?.file === "config.js")).toBe(true);
  ...
  it("tracks one finding once, whichever scope or path form found it", () => {
    dir = mktemp();
    writeFileSync(join(dir, "package.json"), JSON.stringify({ name: "fixture" }));
    const target = join(dir, "config.js");
    writeFileSync(target, 'const apiKey = "sk_live_1234567890abcdef";\n');
    const review = (scope: Parameters<typeof executeReview>[0]["scope"]) =>
      executeReview({
        rootDir: dir as string,
        scope,
        depth: "full",
        packIds: ["debuggatha/release"],
        policyId: undefined,
    ...
    const first = review({ kind: "files", paths: [target] });
    review({ kind: "workspace" });
    review({ kind: "files", paths: [target] });

    const persisted = JSON.parse(readFileSync(join(dir, ".debuggatha", "ledger.json"), "utf8"));
    expect(persisted.entries.length).toBe(first.result.findings.length);
    expect(persisted.entries.every((e: { status: string }) => e.status === "open")).toBe(true);
  });

  it("keeps a file outside the repository absolute", () => {
    expect(toRepoRelative("/repo", "/elsewhere/a.ts")).toBe("/elsewhere/a.ts");
    expect(toRepoRelative("/repo", "src/a.ts")).toBe("src/a.ts");
  });

  it("detects applicable packs on its own when none are named", () => {
    dir = mktemp();
    writeFileSync(join(dir, "package.json"), JSON.stringify({ name: "fixture" }));
    const target = join(dir, "config.js");
    writeFileSync(target, 'const apiKey = "sk_live_1234567890abcdef";\n');
    ...
    expect(result.summary.appliedPacks.length).toBeGreaterThan(0);
    expect(result.findings.length).toBeGreaterThan(0);
  });

  it("leaves the ledger untouched when persist is false", () => {
    dir = mktemp();
    writeFileSync(join(dir, "package.json"), JSON.stringify({ name: "fixture" }));
    const target = join(dir, "config.js");
    writeFileSync(target, 'const apiKey = "sk_live_1234567890abcdef";\n');
    ...
    expect(result.findings.length).toBeGreaterThan(0);
    expect(syncReport.newEntryIds.length).toBe(result.findings.length);
    expect(existsSync(join(dir, ".debuggatha", "ledger.json"))).toBe(false);
  });

  it("puts explicitly named packs ahead of detected ones", () => {
    const ids = real.resolveRequestedPackIds(["debuggatha/owasp"]);
    expect(ids[0]).toBe("debuggatha/owasp");
...
describe("executeReview — capabilities", () => {
...
describe("executeReview — Repository Memory", () => {
