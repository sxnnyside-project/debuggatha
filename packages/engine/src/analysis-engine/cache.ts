...
/**
 * Same shape/strategy as `core/repository-intelligence`'s own
 * fingerprint cache — a separate, independent implementation (not a
 * shared dependency: this engine must not duplicate Repository Intelligence
 ...
 * a tracked file's content changing) — mirroring exactly why the
...
/** Injectable, in-memory — same discipline as `RepositoryContextCache`: no hidden global singleton, caller controls lifetime/scope. */
