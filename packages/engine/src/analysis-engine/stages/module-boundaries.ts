...
import { listScanFiles } from "../../scan-scope/index.js";
import { IGNORED_DIRS, readJson, SOURCE_EXTENSIONS } from "../internal/fs-walk.js";
...
/**
 * Module Boundaries. Respects whatever Repository Intelligence
...
export function inferModuleBoundaries(rootDir: string): ModuleBoundary[] {
  ...
  const scanned = listScanFiles(rootDir, SOURCE_EXTENSIONS);
  const filesUnder = (dir: string) => {
    const prefix = `${relative(rootDir, dir).split(sep).join("/")}/`;
    return scanned.filter((file) => file.startsWith(prefix));
  };
