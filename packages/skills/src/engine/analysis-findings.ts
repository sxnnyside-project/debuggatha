import { runAnalysis } from "@debuggatha/analysis-engine";
import { createFinding, type Evidence, type Finding } from "@debuggatha/review-engine";

/**
 * Turns `@debuggatha/analysis-engine`'s deterministic repository analysis
 * (Epic 12) into `Finding[]` for Architecture Review — this Skill
 * consumes the Analysis Engine's own dependency graph/layer model
 * instead of re-walking the repository's imports itself (Epic 12
 * "Review Skills should consume analysis results instead of recomputing
 * them... avoid duplicated graph traversals"). This replaces Epic 11's
 * bespoke `detectDependencyDirectionViolations`, which duplicated exactly
 * this traversal.
 */
