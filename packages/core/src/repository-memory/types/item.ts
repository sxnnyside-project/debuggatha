...
/**
 * Never conflated with `engine/context-intelligence`'s
 * `ContextConfidence`:
 * this is memory's own three levels, because memory is about
...
interface MemoryItemBase {
  ...
  createdAt: string;
  updatedAt: string;
  /** Required, never optional — "users should always understand why Debuggatha remembers something". */
  rationale: string;
 ...
 * intentionally mirrors `core/findings-ledger`'s
...
/** Repository-specific convention — "complements Review Packs rather than replace them": never a substitute for pack content, an addition to it. */
 ...
 * see `core/findings-ledger`'s `HistoryEvent[]`).
 */
